...
mask_train= np.zeros([batch_num_count,Batch_Size,Mask_Height,Mask_Width])
print('Initialized Input Video Frames Shape = ' + str(frames_train.shape))
print('Initialized Mask Images Shape = ' + str(mask_train.shape))
print('Initialized rPPG labels Shape = ' + str(labels_train.shape))


# Load Training Video Frames and Mask Images
...
for batch_num in range(batch_num_count):
        for i in range(Batch_Size):
            ...
            print(img_counter)
        frames_train[batch_num,:,:,:,:] = np.rollaxis(images,axis=-1)
        labels_train[batch_num,:] = np.reshape(rppg_data_label[batch_num*Batch_Size:batch_num*Batch_Size + Batch_Size],[1,Batch_Size])
        mask_train[batch_num,:,:,:] = np.rollaxis(mask_images,axis=-1)
      
print('Frames Train Generated')
print('Mask images Train Generated')
...
for epoch_num in range(EPOCHS):
  ...
  for i in range(batch_num_count):
      ...
      Skin_Mask_Images = np.reshape(mask_train[batch_num,:],[1,Batch_Size, Mask_Height,Mask_Width])
      ...
      skin_map, rPPG_aux, rPPG, rPPG_SA1, rPPG_SA2, rPPG_SA3, rPPG_SA4, x_visual6464, x_visual3232  = model(Input_Frames)
  ...
  torch.save(model.state_dict(), '/content/drive/My Drive/stevenrppgmodel.pt')
